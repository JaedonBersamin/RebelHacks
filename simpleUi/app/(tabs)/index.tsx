...
export default function App() {
  ...
  const todayEvents = liveEvents.filter((event) => {
    ...
    const eventDateStr = event.time.split(" at ")[0];
  ...
  // ── Fetch hotspots (used on mount and when map opens) ──────────────────────
  ...
  // ── Refresh hotspots every time the map is opened ─────────────────────────
  useEffect(() => {
    if (showMap) {
      fetchHotspots();
    }
  ...
  // ── Get location & open modal ──────────────────────────────────────────────
  const handleReportSolicitor = async () => {
    if (!locationPermission) {
      Alert.alert("Permission Denied", "We need your location to drop a pin!");
      return;
    }
    setIsReporting(true);
    try {
  ...
  // ── Submit new hotspot with optimistic UI ──────────────────────────────────
  const handleSubmitHotspot = async () => {
    ...
    setHotSpots((spots) => [...spots, optimisticSpot]);
    setMyHotspotId(tempId); // treat as "owner" straight away

    try {
      const newWarningPin = {
        latitude: pendingLocation.latitude,
        longitude: pendingLocation.longitude,
        timestamp: optimisticSpot.timestamp,
      ...
      setHotSpots((spots) => spots.filter((s) => s.id !== tempId));
      setMyHotspotId(null);
